...
class Graph():
    def __init__(self):
        ...
        self.edge_dic = {}
        self.node_type_id = defaultdict(lambda: [])  # 各类型节点集合

        self.node_feature = defaultdict(lambda: [])
        self.node_type = []        # 0代表微博，1代表用户,下标+1对应节点序号
        self.edge_index = []       # [[源节点序号,目标节点序号],...] ，从1开始
        self.edge_type = []
        self.edges_weight = []
        self.X_all_content=[]    # 所有推文内容，按dic序号排
        '''
    ...
    def add_node(self, node,type):
        if node not in self.node_type_id[type]:
            self.node_type_id[type].append(node)
            return node
        return node

    def add_edge(self, target_type, source_type, target_node, source_node,  relation_type = None, directed = False):
        edge = [self.add_node(source_node, source_type), self.add_node(target_node, target_type)]
        '''
            Add bi-directional edges with different relation type
        '''
        self.edge_list[target_type][source_type][relation_type][edge[1]].append(edge[0])
        if directed:
            self.edge_list[target_type][source_type]['rev_' + relation_type][edge[1]].append(edge[0])
        else:
            self.edge_list[source_type][target_type][relation_type][edge[1]].append(edge[0])
    ...
    # 总共多少节点类型
    def get_types(self):
        return list(self.node_type_id.keys())
