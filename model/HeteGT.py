...
class HeteGTConv(MessagePassing):
    ...
    def message(self, edge_index_i, node_inp_i, node_inp_j, node_type_i, node_type_j, edge_type):
        ...
        '''
        # res_att = torch.zeros(data_size, self.n_heads).to(self.device)
        # res_msg = torch.zeros(data_size, self.n_heads, self.d_k).to(self.device)
        ...
        for source_type in range(self.num_types):
            ...
            for target_type in range(self.num_types):
                tb = (node_type_i == int(target_type)) & sb    # 目标节点与源节点类型是否相等
    ...
    def update(self, aggr_out, node_inp, node_type):
        ...
        for target_type in range(self.num_types):
            ...
            if self.use_norm:
                ...
                res[idx] = trans_out + node_inp[idx]
        return res
