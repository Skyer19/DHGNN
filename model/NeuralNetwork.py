...
class NeuralNetwork(nn.Module):
    ...
    def train_step(self, i, data):
        with torch.no_grad():
            batch_tid, batch_source, batch_replies, batch_y = (item.to(self.device) for item in data)

        self.optimizer.zero_grad()
        logit = self(batch_tid, batch_source, batch_replies)
    ...
    def fit(self, X_train_tid, X_train_source, X_train_replies, y_train,
        ...
        for epoch in range(self.config['epochs']):
            ...
            cnt = y_train.size(0) // batch_size + 1  # size()矩阵某一维上的元素个数的函数,0为行数，1为列数
            print("Average loss:{:.6f} average acc:{:.6f}%".format(avg_loss/cnt, avg_acc/cnt))
            if epoch >= 10 and self.patience > 3:                        # 调整学习率
    ...
    def evaluate(self, X_dev_tid, X_dev_source, X_dev_replies, y_dev):
        ...
        print(classification_report(y_dev, y_pred, target_names=self.config['target_names'], digits=5))   # 计算f1 recall

        if acc > self.best_acc:
            self.best_acc = acc
            self.patience = 0
            torch.save(self.state_dict(), self.config['save_path'],_use_new_zipfile_serialization=False)
    ...
    def predict(self, X_test_tid, X_test_source, X_test_replies):
        ...
        for i, data in enumerate(dataloader):  # twitter15:149,twitter:28
            with torch.no_grad():
                batch_tid, batch_source, batch_replies = (item.to(self.device) for item in data)
